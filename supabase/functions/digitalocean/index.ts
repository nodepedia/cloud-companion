    ...
    switch (action) {
      ...
      case 'get-console': {
        ...
        if (droplet.status !== 'active') {
          throw new Error('Droplet harus dalam status aktif untuk mengakses console');
        }

        // Request console access from DigitalOcean
        const response = await doRequestWithFailover(
          supabase, 
          `/droplets/${droplet.digitalocean_id}/console`, 
          'GET'
        );
        ...
        result = { 
          console_url: response.data.console?.url,
          droplet_name: droplet.name
        };
        break;
      }

      case 'admin-get-console': {
        const { dropletId } = params;
